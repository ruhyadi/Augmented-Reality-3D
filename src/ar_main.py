...
def main():
    ...
    dir_name = os.getcwd()
    model = cv2.imread(os.path.join(dir_name, 'reference/blueeyes.png'), 0)
    # Compute model keypoints and its descriptors
    kp_model, des_model = orb.detectAndCompute(model, None)
    # Load 3D model from OBJ file
    obj = OBJ(os.path.join(dir_name, 'models/blueeyes.obj'), swapyz=True)
    # init video capture
    cap = cv2.VideoCapture('/home/didi/Repository/Nodeflux/AR-OpenCV/blue_eyes2.mp4')
...
def render(img, obj, projection, model, color=True):
    ...
    for face in obj.faces:
        ...
        # elif color==True:
        color_value = tuple([int(i/1 * 255) for i in mtl[face[-1]]['Kd']])
        # color = hex_to_rgb(face[-1])
        color_value = color_value[::-1] # reverse
        cv2.fillConvexPoly(img, imgpts, color_value)
